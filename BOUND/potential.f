...
c==============================================================================
	...
	implicit none
     ...
     $  iQB, iQB1, iQB2, iBlock, NR1, NR2, ms, ierr

        integer numCol, numColLeft, tmp, bCol
	...
	do m=1, NRGRIDS
            ...
            if (minval(WK1).LT.VCUT-0.0001d0) then 
              ...
              iBlock=iBlock+1
              write(100) (WK1(iQB),iQB=1,NB1node*NB2node)
            end if 
...
c=======================================================================
	...
	use fileIO
     ...
     .    NA1node, NA2node, NA3node, NB1node, NB2node, VCUT, NBlock,
     .    nodeax,nodebx
	...
	implicit none
        ...
        real*8 Z,R1,R2,R3,A1,A2,A3,B1,B2,PI, CM

        integer i,j,k,l,m,n1,n2,iQA,iQA1,iQA2,iQA3,
     $  iQB,iQB1,iQB2,ierr,NR1,NR2,iBlock, item

	real*8, allocatable :: WK(:,:), tWK(:,:)

	real*8, allocatable :: WK1(:,:), tWK1(:,:)
	...
	allocate(WK(NB2node, NB1node), stat=ierr)

	allocate(WK1(NZVB, NVB2), stat=ierr)
	...
	iBlock=0
     ...
     .      tWK(iQB2,iQB1)*CM 
        ...
        do l=1, NZVB
        do j=1, NVB2
           write(300,*) ZQ(l),RQ2(j),tWK1(l,j)*CM 
        end do
        end do
        close(300)
