c====================================================================
        subroutine HMAT(WA,WT)
        use COMPARAS, only : NZVB,NVB1,NVB2,NVB3,NZABS,NR1ABS,
     .    NANODES,NRGRIDS,NBASJJ,NAp,J1max,J2max,J3max,J23max,
     .    KB1max,KM2max,NA1node,NA2node,NA3node,nodebx,
     .    VCUT, KCUT,IDABS
        use TRANSMATS, only : TMATZ, TMATR1, TMATR2, TMATR3
        use ENGARRAYS, only : EIG4D, FZABS, FR1ABS
        use SCALES, only : Hminus, Hplus
        ...
        NLARGE=2000*max(NZVB,NVB1,NVB2*NVB3)*2
        allocate(CWK(NLARGE),stat=ierr)
     ...
     .    NA2node*NA3node,NBASJJ)
        ...
        do i=1, NRGRIDS
          E4=EIG4D(i)/Hminus
          call DAXPY(NAp, E4, WT((i-1)*NAp+1),INC,
     .      WA((i-1)*NAp+1),INC)
        end do
...
c=====================================================================
	...
	use COMPARAS, only : NBASJJ,J1max,J2max,J3max,J23max, 
     ...
     .    NVB3,NB1node,NB2node, RCUT, NRGRIDS
...
! multiple centrifugal potential Jtot=0
	do ia=1, NBASJJ
     ...
     .    j3Val*(j3Val+1)/BROT_R3(iR3)
          if(BROT.le.RCUT) then
	    WK2(ia)=WK(ia,iR)*BROT/Hminus
          else
	    WK2(ia)=WK(ia,iR)*RCUT/Hminus
          end if
	end do
	...
	do ia=1, NA1node*NA2node*NA3node
	  ...
	  if (LINK(ia,ms).eq.0) then
	    do ib=1, ishift
              WK3((ia-1)*ishift+ib)=
     .          WK3((ia-1)*ishift+ib)*(VCUT-VRef4D(ms)-Hplus)/Hminus
	    end do
!
	  else
            call B1B2MAT0(ms,ia,
...
c====================================================================
	...
	use COMPARAS, only : J1max,J2max,J3max,J23max,JS1,JS2,JS3, 
...
c====================================================================
	...
	use COMPARAS, only : J1max,J2max,J3max,JS1,JS2,JS3, 
...
c====================================================================
	...
	do ia1=1, NA1node
	do kbVal=0, KB1max
	do kmVal=max(-KM2max,-j3val),min(KM2max,j3val)
...
c====================================================================
	...
	do i=1, NB2node
	  ...
	  WK4(i,j)=WK4(i,j)*(VPOT9D(ms,POS_LIST(ia,iR))-Hplus)/Hminus
	end do
	end do
